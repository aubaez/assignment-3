...
int main(int argc, char *argv[])
{
    ...
    host_c.data = (double *)malloc(sizeof(double) * host_c.shape[0] * host_c.shape[1]);
    ...
    device_a = clCreateBuffer(context, CL_MEM_READ_ONLY, host_a.shape[0]*host_a.shape[1]*sizeof(double),NULL,&err);
    CHECK_ERR(err,"clCreateBuffer device_a");
    device_b = clCreateBuffer(context, CL_MEM_READ_ONLY, host_b.shape[0]*host_b.shape[1]*sizeof(double),NULL,&err);
    CHECK_ERR(err,"clCreateBuffer device_b");
    device_c = clCreateBuffer(context, CL_MEM_WRITE_ONLY, host_c.shape[0]*host_c.shape[1]*sizeof(double),NULL,&err);
    CHECK_ERR(err,"clCreateBuffer device_c");
    
    //@@ Copy memory to the GPU here
    err = clEnqueueWriteBuffer(queue,device_a,CL_TRUE,0,host_a.shape[0]*host_a.shape[1]*sizeof(double),host_a.data,0,NULL,NULL);
    CHECK_ERR(err,"clEnqueueWriteBuffer device_a");
    err = clEnqueueWriteBuffer(queue,device_b,CL_TRUE,0,host_b.shape[0]*host_b.shape[1]*sizeof(double),host_b.data,0,NULL,NULL);
    ...
    err = clEnqueueReadBuffer(queue,device_a,CL_TRUE,0,host_a.shape[0]*host_a.shape[1]*sizeof(double),host_a.data,0, NULL, NULL);
    CHECK_ERR(err,"clEnqueueCopyBuffer host_a");
    err = clEnqueueReadBuffer(queue,device_b,CL_TRUE,0,host_b.shape[0]*host_b.shape[1]*sizeof(double),host_b.data,0, NULL, NULL);
    CHECK_ERR(err,"clEnqueueCopyBuffer host_b");
    err = clEnqueueReadBuffer(queue,device_c,CL_TRUE,0,host_c.shape[0]*host_c.shape[1]*sizeof(double),host_c.data,0, NULL, NULL);
